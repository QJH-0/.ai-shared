...
> **决策结果（2026-09-24 19:31）**：用户**采纳全部推荐项** —— #1 不动并登记为预期、#2 保留 + 划边界、#8/#9/#11 修正 README + 登记本地扩展、#12~#16 全部不动。执行记录见「十一」。
>
> 下列各项保留推荐与理由，作为后续复议的依据。
...
## 十一、决策执行记录（2026-09-24）

| 项 | 决策 | 落地位置 |
|---|---|---|
| #1 内嵌副本 | 不动，登记为预期 | `skills/README.md`「已知重名与命名不一致」表 |
| #2 `create-skill` | 保留 + 划边界 | `skills/create-skill/SKILL.md` description 新增边界句 |
| #8 `repo-wiki` 展平 | 文档化 | `skills/README.md`「更新方法 → 本地改动的具体内容」 |
| #9 `fireworks-tech-graph` 本地扩展 | 登记为「禁止直接 pull」 | 同上；含改动文件、`body` 字段语义、动机、`NODE_LINE_HEIGHT` 契约 |
| #11 README 更新指令 | 按克隆分三类改写 | `skills/README.md`「更新方法 → GitHub 克隆的 Skills（7 个）」 |
| #3 #4 #5 #6 #7 | 已在 §九 完成 | — |
| #10 #12 #13 #14 #15 #16 | 按推荐不动 | 理由见 §十 |

**结论**：本轮共处理 5 项（#1 #2 #8 #9 #11），其余 11 项经判定无需动作。`skills/` 未删除或移动任何文件，唯一的内容改动是 `create-skill` 的一句边界说明。
