...
**但读写分开只解决"源不被改写"，不解决"写入目录跨轮累积"**——这是一个独立的坑，踩过：

写入目录是**跨轮累积**的，而"这次跑哪些阶段"由 `resume_from_stage` 决定，两者是**两个正交的事**。
于是 `resume_from_stage='stage8'` 会静默地把 S8 建在上一轮遗留的 S1–S7 上：整条阶梯混代，
却没有任何提示，而下游结论（含蒸馏教师）全建在旧产物上。两个必加机制：

- **开跑前打印阶梯报告**，判据用**保存时间倒挂**（`stage_k` 的最后保存时间早于 `stage_{k-1}`
  ⇒ 它不是在这份前一阶段上训出来的），精确、无需阈值。**不要用"指标倒退"当判据**——
  真实的量化断崖（如 A2→A1 掉 4 dB）会被误报成异常。
- **声明式重跑开关**（如 `SPEX_RERUN_FROM_STAGE=stage1`）：声明"从这里起是全新一轮"，
  该阶段起的旧产物**归档挪走**（挪走而非删除）、不再从续训源导入、阶段下限一并抬到该阶段。
  **`resume_from_stage=X` 单独用表达不出"重跑"**——那个阶段会从自己的 `_latest` 接着训，
  旧阶梯原样保留；这是必须另加开关的根本理由。

**归档时务必查"还有谁会把它找回来"**。踩过：阶段 best 有**两种落点**——`checkpoints/<stage>/`
目录内一份，`EXP_DIR/<stage>_best.pt` 扁平一份（保存时 `is_best` 两份都写）；而
`find_latest_checkpoint()` 的兜底正是读扁平那份。只挪目录盖不住它 ⇒ `run_stage` 仍命中旧
checkpoint ⇒ **"重跑"静默变成"接着训"，比不改还糟**（表面有归档日志、实际没重跑）。
两个落点、两个查找函数（一个只看目录内、一个带扁平兜底）都要覆盖，并加一条**集成断言**：
归档后用生产代码自己的 `find_latest_checkpoint()` 断言该阶段返回 `None`。

**"重跑"是否干净，还要看下游教师/依赖产物的生命周期**。本项目两个教师生命周期不同：
输出级 FP32 教师从共享源（S0 best）**循环外构建一次**（合理，S0 不参与重跑）；
特征级量化教师 `build_feat_teacher(stage)` 在**循环内每阶段重建**——所以重训出的新
stage_{k-1} 会被 stage_k 立即读到，不存在隐性混代。**若日后把它提到循环外做缓存，
"重跑"会静默降级成半混代**，改那里必须同步更新这条约束。
...
**装之前先探测，装齐就完全不碰 pip**。脚本会被反复执行——notebook 每次重跑、批量 runner
每个实验臂都跑一遍完整 notebook——而 pip 即使"Requirement already satisfied"也要走一遍索引解析，
网络差或断网时这一趟会超时，把整个安装 cell 拖失败。做法：用 `importlib.metadata.version`
（与自检同一口径）算出缺哪几个，两个数组都空就直接打印"依赖已齐"返回；否则只把缺的包交给 pip。
`pip --version` 的存在性检查也一并挪进"真要装"分支——装齐的机器没有 pip 不影响训练。
...
替身要按"脚本真正会问的问题"分流：`sys.executable` / `sys.version.split` / `EXTERNALLY-MANAGED` /
`m.version('<pkg>')`（由环境变量决定哪些包"已装"）、`-` 走 heredoc 分支、`-m pip ...` 记日志后
按开关决定成败。这样能真实跑通控制流而**不装任何包、不碰网络**；覆盖四类分支
（全齐 / 只缺必需 / 只缺可选 / 全缺）×（pip 成功 / pip 失败）×（带 `--check` / 不带）就够了。
**计数时按 `pip install` 过滤**——替身会把每次探测子进程也写进日志，把探测次数当成 pip 次数
会得出完全相反的结论（"装齐时仍调了 18 次 pip"）。
...
| 续训后**前一阶段指标比后一阶段还高**、结论与上次对不上 | 写入目录**跨轮累积**：只跑了末尾阶段，前面阶段是上一轮遗留 ⇒ 阶梯混代（`resume_from_stage` 只决定跑哪些，不决定哪些算全新） | 开跑打印阶梯报告（**保存时间倒挂**判据，不用指标差）；要重跑就用声明式重跑开关，别单用 `resume_from_stage` |
| 设了"重跑"却没重跑，日志却显示归档成功 | 阶段 best 有两种落点（目录内 + `EXP_DIR/<stage>_best.pt` 扁平），归档只挪了目录，扁平那份被 `find_latest_checkpoint()` 的兜底捞回 | 归档要覆盖**两种落点**；加集成断言：归档后 `find_latest_checkpoint()` 该阶段返回 `None` |
| 覆盖上传了新 notebook 但自检仍报 `[MISS]` | 现场首格是旧版本（或被手改），"回显意图"的打印盖住了真实生效值 | 覆盖上传 notebook 本身（不只是 `.sh`）；自检要读 **cfg 实际生效值**，而不是回显你以为设进去的值 |
