...
- **服务端幂等语义变更：`code=10001`（今日已签到）改从 HTTP 200 变为 HTTP 400 返回**。`checkin.js` / `checkin.sh` 原先在非 200 时直接判失败（exit 4），导致「明明已签到成功」却被汇报为「签到失败：HTTP 400」。现改为：非 200 时先解析响应体，命中 `code=10001` 仍按幂等成功处理（"今日已签到，幂等跳过"）。实测于 2026-09-06 ~ 09-07，`checkin-status` 两路径仍为 200。
  - 排查过程：`checkin-status` 200 正常、`daily-checkin` 400，响应体 `{"code":10001,"msg":"今天已签到，请明天再来"}`——即网关/服务端把业务错误改用 HTTP 400 承载，与 1.0.3 的「真实 HTTP 状态码判定」并不冲突，只是幂等分支需要看 body 而非状态码。
