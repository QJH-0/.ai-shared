...
`type(scope):` 保持 Conventional Commits 常见英文 type 与 kebab-case scope，**冒号后默认用中文**一行说明：

```
feat(api): 新增视频元数据接口
fix(auth): 使用安全比较修复密钥校验时序问题
test(security): 增加路径穿越防护回归用例
refactor(template): 合并文件名清洗逻辑
```

若仓库的 `CLAUDE.md`、`.cursor/rules` 或贡献指南要求英文标题，则首行改为英文即可；下方各类型英文示例仍可作为 **正文（body）** 的写法参考。
...
fix(auth): 使用 hmac 安全比较校验 API 密钥
refactor(template): 合并文件名清洗相关逻辑
test(security): 增加路径穿越防护测试用例
```
