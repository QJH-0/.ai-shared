...
**⚠️ Critical: verify Heading style definitions exist in `styles.xml` before adding a TOC.** When a document is created from scratch (not from a `.dotx` template), `styles.xml` may contain only the `Normal` style — Heading1/2/3 are *referenced* by paragraphs but never *defined*. Word cannot build a TOC from undefined styles: the TOC field renders empty, the navigation pane shows no headings, and `updateFields=true` does not help because there are no outline-level paragraphs to collect. Always check with `officecli get "$FILE" /styles --depth 1` — if only `Normal` appears, add the missing style definitions before inserting the TOC:

```bash
# Step 1: verify styles exist. If Heading1/2/3 are missing, add them:
officecli add "$FILE" /styles --type style --prop id=Heading1 --prop type=paragraph --prop name="heading 1" --prop outlineLvl=0 --prop size=20pt --prop bold=true
officecli add "$FILE" /styles --type style --prop id=Heading2 --prop type=paragraph --prop name="heading 2" --prop outlineLvl=1 --prop size=14pt --prop bold=true
officecli add "$FILE" /styles --type style --prop id=Heading3 --prop type=paragraph --prop name="heading 3" --prop outlineLvl=2 --prop size=12pt --prop bold=true
# Step 2: verify headings in the body reference these styles.
officecli add "$FILE" /body --type paragraph --prop text="Introduction" --prop style=Heading1
# Custom paragraph styles also need an outline level (0 = Heading 1).
officecli add "$FILE" /styles --type style --prop id=ThesisH1 --prop type=paragraph --prop outlineLvl=0
# Step 3: add the TOC only after styles exist AND headings reference them.
officecli add "$FILE" /body --type toc --prop levels="1-3" --prop title="Table of Contents" --prop hyperlinks=true --prop pageNumbers=true
```

**`levels` format: must use a hyphen range (e.g. `"1-3"`), not a bare number (e.g. `"3"`).** A bare `3` generates `TOC \o "3"` which only collects Heading level 3, not levels 1–3. The correct instruction is `TOC \o "1-3"`. Verify with `get /toc[1]` — the `text` field should read `TOC \o "1-3" \h \u`.
...
| TOC renders empty in Word | Check `get /styles --depth 1` — Heading1/2/3 must be *defined* in styles.xml, not just referenced by paragraphs; add them with `add /styles --type style --prop id=Heading1 …` if missing |
| TOC `levels="3"` only collects H3 | Use `levels="1-3"` (hyphen range); bare `3` generates `TOC \o "3"` which misses H1/H2 |
