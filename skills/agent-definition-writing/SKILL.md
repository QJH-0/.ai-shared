...
- [ ] 多 agent 间协作机制（门禁、报告合并规则）双方定义一致；**共享机制的判定规则单源**——只在一处定义，其余文件引用，禁止多文件各写一份
- [ ] 状态机**穷尽无空洞**：判定表覆盖所有取值组合；合并规则写成「全序取最严格」而非列举组合（列举必然留空洞）；每个状态都要有下游消费点（谁读、读到怎么处理）
- [ ] 定义内的命令示例**自包含**：不依赖跨调用存活的 shell 变量（`BASE=$(...)` 后续用 `$BASE` 会静默变空）、不依赖隐式目录状态
