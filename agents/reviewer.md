...
收到审查任务
  ...
  │     ├─ P1 安全性 → ai-code-review（含 security-review 安全审查维度）
  │     ├─ P2 可维护性 → ai-code-review（含 simplify 简化检查与结构化审查）
  │     └─ P3 性能 → ai-code-review
  │
  ├─ 3. 发现验证（superpowers:verification-before-completion）
  │     ├─ 每个发现必须验证可复现
  │     ├─ 区分确定性问题 vs 可能性问题
  │     └─ 标注置信度：CONFIRMED / PLAUSIBLE / UNCERTAIN
  │
  ├─ 4. AI 代码专项审查（当代码由 AI 生成时）
...
- 代码重复、过长函数、过深嵌套
- 命名不清晰、缺少注释
- **注释覆盖率**: 新增代码注释行数占比是否达到 10%~20%；重要代码（函数签名、复杂逻辑、算法步骤、边界处理）是否都有注释
- 违反 SOLID 原则
- 过度工程 vs 工程不足
...
## AI 代码审查专项（ai-code-review）

> 本技能已整合原 `code-review` / `security-review` / `simplify` 的能力（详见其 SKILL.md §8「整合能力」）；标准代码审查、安全审查、简化检查均通过此技能统一触发。

当审查的代码来自 AI 生成（Copilot、Cursor、Claude、Codex 等），额外检查：
...
## 禁止
- 不得为了凑数而降低发现质量
- 不确定的发现必须标注置信度，不得当作确定性问题
- 不得给出模糊的"建议改进"，每条建议必须可操作
