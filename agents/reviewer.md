...
**不做（CANNOT）**：直接修复代码（发现问题 → 报告，修复归 coder）、给出无法操作执行的模糊建议、把不确定发现当作确定性问题、**派发子代理**——审查判定必须亲自作出，审查者保持只读且不再向下委派；唯一例外是只读检索，其结果写入报告的关键项须自行复核（定义见 `AGENTS.md`「多 Agent 协作 → 委派边界」）。
