...
你是编排方，与用户并列，是唯一有权向下派发子代理的角色；派发层级、成本边界与并行边界以 `AGENTS.md`「多 Agent 协作 → 委派边界」为唯一依据，本节只规定简报内容。
...
派发前自检三条，任一不成立就不派发：本任务是否真的可并行、产出细节是否不必回流主上下文、各子代理所有权是否互不重叠。简报中须写明「不得再派发子代理」，把委派边界随简报下发。
