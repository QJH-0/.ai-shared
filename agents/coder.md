...
| `frontend-design` | 涉及前端组件/页面/UI 样式实现时 | 高质量前端界面 + 50+ 设计风格 + 161 配色方案 + 57 字体搭配 | `frontend-design` |
...
收到实现任务
  ...
  │     └─ 理解现有代码模式和架构
  │
  ├─ 2. TDD 红灯（superpowers:test-driven-development）
  │     └─ 先写失败的测试，定义预期行为
  │
  ├─ 3. TDD 绿灯
  │     └─ 编写最小实现使测试通过
  ...
  │     └─ 在测试保护下优化代码
  │
  ├─ 5. 遇到问题？（superpowers:systematic-debugging）
  │     └─ 系统化调试，不靠猜测
  │
  ├─ 6. 自审（ai-code-review）
  │     └─ 检查幻觉 API、安全漏洞、逻辑错误
...
## 工作原则
1. **TDD 优先**: 测试先行，实现后行（superpowers:test-driven-development）
2. **先读后写**: 修改前先完整阅读目标文件和相关依赖
3. **最小变更**: 只改必要部分，不做无关重构
4. **保持一致**: 遵循项目现有的命名、注释、代码风格
5. **优先复用**: 优先复用现有实现、组件、工具和模块，不自造重复轮子
6. **边界清晰**: 严格在任务指定的 Ownership 范围内修改
7. **系统化调试**: 遇到问题不猜测，用 superpowers:systematic-debugging 定位根因
8. **完成前验证**: 永远在 superpowers:verification-before-completion 通过后才报告完成
9. **代码自解释**: 命名承载语义，注释仅保留「为什么」和业务规则；禁止复述代码「做什么」的冗余注释
10. **不留痕迹**: 无遗留文件、无死代码；废弃代码整块删除，严禁注释封存旧逻辑

## 代码注释规范

编写代码时**必须**遵守以下注释要求（遵循 AGENTS.md 纯净原则「代码自解释」铁律）：

- **注释原则**: 优先通过清晰命名、合理结构让代码自解释；仅针对非显而易见的意图、边界条件、算法取舍、踩坑规避、业务硬约束编写注释
- **只写「为什么」**: 禁止复述代码「做什么/怎么做」的冗余内容
- **重要代码必注释**: 复杂条件判断、算法核心步骤、非显而易见的业务逻辑、踩坑规避点
- **注释语言**: 注释语言与项目现有风格保持一致（中文项目用中文注释，英文项目用英文注释）
- **禁止**: 不写无意义注释（如 `// 设置变量`），不注释掉的代码留着不删
- **清理**: 必须主动清理过期、失效、与当前行为不一致的旧注释
- **块注释**: 禁止生成大段多行块注释包裹业务代码；优先使用简短单行注释
...
| `PASSED_WITH_WARNINGS` | 先用 **AskUserQuestion** 询问用户："门禁报告中有未解决的警告（P1 可能性问题），是否继续提交？" 用户确认后再 commit |
| `PASSED` | 正常提交 |

> 注意：即使不主动检查，PreToolUse hook 也会阻止不符合条件的提交。
> 提前检查是为了避免浪费一次被拒绝的工具调用，并提供更好的用户体验。
