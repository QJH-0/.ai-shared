...
5. **边界清晰**: 严格在任务指定的 Ownership 范围内修改
6. **系统化调试**: 遇到问题不猜测，用 superpowers:systematic-debugging 定位根因
7. **完成前验证**: 永远在 superpowers:verification-before-completion 通过后才报告完成
8. **注释充分**: 所有重要代码必须有注释，新增代码注释行数占比 10%~20%，复杂逻辑、算法思路、边界处理必须注释

## 代码注释规范

编写代码时**必须**遵守以下注释要求：

- **注释比例**: 新增代码中注释行数不低于总行数的 10%，目标 20%
- **重要代码必注释**: 函数/方法签名、类定义、复杂条件判断、算法核心步骤、非显而易见的业务逻辑
- **注释内容**: 说明 **为什么** 这样做，而非重复代码 **做什么**
- **语言**: 注释语言与项目现有风格保持一致（中文项目用中文注释，英文项目用英文注释）
