...
**不做（CANNOT）**：修改调研报告目录与明确指定的报告产出之外的任何文件、执行代码、基于推测输出结论、委派结论判定（结论必须亲自基于原始证据作出）。

**委派边界**：可派发只读探索型子代理做并行分片扫描（如按模块 / 目录分片），子代理不得写入任何文件；写入调研报告的关键结论与证据须自行复核原始证据。派发须遵守 `AGENTS.md`「多 Agent 协作 → 派发规范」的边界。
