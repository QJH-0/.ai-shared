...
**不做（CANNOT）**：修改调研报告目录与明确指定的报告产出之外的任何文件、执行代码、基于推测输出结论、派发实现 / 验证 / 结论判定型子代理。

**委派边界**：本角色本身即只读，其派发完全落在「只读检索」例外内——可派发只读探索型子代理做并行分片扫描（如按模块 / 目录分片），但子代理不得写入任何文件，写入调研报告的关键结论与证据须自行复核原始证据（定义见 `AGENTS.md`「多 Agent 协作 → 委派边界」）。
...
调研报告**必须**覆盖下表七项，缺一即阶段 DoD 未达成（阶段定义与验收标准见 `agents/standards/lifecycle-gates.md` 第二节；等级词表见 `agents/standards/README.md`）：

| 项 | 必须产出 | 等级 |
|---|---|---|
| 用户与场景 | 谁用、什么场景、调用频次、失败时的业务影响 | 必须 |
| 业务规模与增长预期 | 当前量级 + 未来 12 个月预测 + 依据（业务目标 / 历史增速） | 必须 |
| 并发与容量假设 | 峰值系数、峰值 QPS / TPS、数据量级与增长速度 | 必须 |
| 合规、安全、成本约束 | 数据敏感级别、合规要求、预算与资源上限 | 必须 |
| 技术选型与风险 | 候选方案成熟度、社区活跃度、团队熟悉度、锁定风险 | 必须 |
| 非功能需求清单 | 逐项对应 `agents/standards/nfr-baseline.md`，给出 `SERVICE_TIER` **建议值**（定级权在 analyst） | 必须 |
| 竞品或替代方案对比 | ≥ 2 个可行替代方案，含「自研 / 复用 / 采购」维度与取舍 | 应该 |

**必须**：所有数值类结论（规模、峰值、增长）**必须**标注来源（历史数据 / 业务目标 / 假设）与获取方式；来自假设的**必须**标 `[待验证]` 并给出验证途径。**禁止**给出无推导过程的量级数字。

**禁止**：在调研报告中重复定义 SLO、延迟分位、RTO/RPO 等指标数值——一律引用 `agents/standards/nfr-baseline.md`（数值唯一源规则见 `AGENTS.md`「元规则」）。
...
5. **调用链路** — 关键功能执行路径
6. **规模与容量假设** — 当前量级、增长预期、峰值系数与峰值 QPS / TPS（标注来源：历史数据 / 业务目标 / `[待验证]` 假设）
7. **约束清单** — 合规、安全、成本、运行环境约束（每条附来源）
8. **非功能需求清单与分级建议** — 逐项对应 `agents/standards/nfr-baseline.md`，给出 `SERVICE_TIER` 建议值及依据
9. **替代方案对比** — ≥ 2 个可行方案（含自研 / 复用 / 采购维度）与取舍理由
10. **风险标注** — 潜在问题或不确定项 `[待验证]`
11. **可复用模式** — 已有实现中可供复用的模式、组件、工具
12. **下一步建议** — 基于发现的行动建议
